...
def build_manifests(df: pd.DataFrame) -> dict:
    ...
    lists per split. Returns dict keyed by split name, each containing 'source' and
    'target' sub-dicts with 'recordings' and 'supervisions' lists.
    """
    manifests = {
        split: {
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Building manifests"):
        ...
        manifests[split]["source"]["recordings"].append(src_recording)
        manifests[split]["source"]["supervisions"].append(src_segment)
        ...
        manifests[split]["target"]["recordings"].append(tgt_recording)
        manifests[split]["target"]["supervisions"].append(tgt_segment)
...
def save_manifests(manifests: dict, output_dir: Path, use_json: bool):
    ...
    for split, sides in manifests.items():
